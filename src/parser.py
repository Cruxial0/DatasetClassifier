...
def tokenize(condition_string: str) -> List[str]:
    ...
    while i < len(condition_string):
        ...
        if condition_string[i:i+3] == 'AND':
            tokens.append('AND')
            i += 3
        elif condition_string[i:i+2] == 'OR':
            tokens.append('OR')
            i += 2
        elif condition_string[i:i+3] == 'NOT':
            tokens.append('NOT')
            i += 3
        elif condition_string[i:i+2] == '&&':
            tokens.append('AND')
            i += 2
        elif condition_string[i:i+2] == '||':
            tokens.append('OR')
...
def evaluate_single_condition(condition: Condition, image_tags: List[int],
    ...
    if condition.condition_type == ConditionType.COMPLETED:
        ...
        # Compare count of selected tags
        count = len(selected_tags)
        
        if condition.comparison == ComparisonOp.EQUAL:
            return count == condition.count
        elif condition.comparison == ComparisonOp.GREATER:
            return count > condition.count
        elif condition.comparison == ComparisonOp.GREATER_EQUAL:
            return count >= condition.count
        elif condition.comparison == ComparisonOp.LESS:
            return count < condition.count
        elif condition.comparison == ComparisonOp.LESS_EQUAL:
            return count <= condition.count
